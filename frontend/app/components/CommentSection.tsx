...
import { StrapiComment } from "@/lib/strapi";

interface Comment {
  id: string;
  author: string;
  content: any; // Bisa string (fallback/optimistic) atau array blocks dari Strapi
  createdAt: string;
  avatarUrl?: string;
}
...
export default function CommentSection({ articleId, initialComments = [] }: CommentSectionProps) {
  const mapStrapiComment = (c: StrapiComment): Comment => {
    const data = c.attributes || c;
    return {
      id: String(c.id),
      author: data.Name || "Anonim",
  ...
  const [comments, setComments] = useState<Comment[]>(() =>
    initialComments.map(mapStrapiComment).sort(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const savedCommentData = await createCommentAction(articleId, author.trim(), content.trim());

      const newComment: Comment = {
        id: String(savedCommentData.id),
        author: savedCommentData.Name || author.trim(),
        content: savedCommentData.Comment || content.trim(),
        createdAt: savedCommentData.createdAt || new Date().toISOString(),
        avatarUrl: "https://upload.wikimedia.org/wikipedia/commons/8/89/Portrait_Placeholder.png",
      };
  ...
  return (
    <section className="mt-16 pt-10 border-t border-zinc-100 dark:border-zinc-800/80">
      ...
      <form
        ...
        className="mb-10 bg-zinc-50/50 dark:bg-zinc-900/20 border border-zinc-100 dark:border-zinc-800/80 rounded-2xl p-6 relative overflow-hidden"
      >
        ...
        <div className="grid grid-cols-1 gap-4 mb-4">
          <div>
            <label
              htmlFor="name-input"
              className="block text-xs font-bold text-zinc-500 dark:text-zinc-400 mb-1.5 uppercase tracking-wider"
            >
              Nama Lengkap
            </label>
            <input
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {typeof comment.content === "string" ? (
                  ...
                  <div className="prose prose-sm prose-zinc dark:prose-invert max-w-none text-sm text-zinc-600 dark:text-zinc-450">
                    {comment.content && <BlocksRenderer content={comment.content} />}
                  </div>
                )}
              </div>
            </div>
